...
RUN_NAME="rnnps-medium-${DATASET_SHORT}-lr${LR}-rlr${RNNPS_LR}-b1_${BETA1}-b2_${BETA2}-wd${WD}-rwd${RNNPS_WD}-it${MAX_ITERS}"
# Log file
...
torchrun --standalone --nproc_per_node=${GPUS} \
      MARS/train_rnnps_streaming_fw.py \
      config/train_gpt2_medium_rnnps_streaming_fw.py \
